...
void MsInterpolation::test()
{
	...
	build_interpolation(face_root, m_mesh, target_mesh, 0.25);
	...
	write_mesh_data( "interpolation_result.obj");
}

void MsInterpolation::test(int n,char *output_path)
{
	while (1)
	{
		release_facenode_help(face_root);
	...
	{
		build_interpolation(face_root, m_mesh, target_mesh, i/n);

		MyMesh::VertexIter v_iter, v_end(m_mesh.vertices_end());
		std::vector<MyMesh::Point> value;
		for (v_iter = m_mesh.vertices_begin(); v_iter != v_end; ++v_iter)
		{
			MyVector3f v = face_root->pts[v_iter.handle().idx()];
			value.emplace_back(m_mesh.point(v_iter));
			m_mesh.set_point(v_iter, MyMesh::Point(v[0], v[1], v[2]));
		}
		char str[800];
		sprintf(str, "_%lf.obj", i / n);
		write_mesh_data(std::string(output_path)+str);
		int j = 0;
		for (v_iter = m_mesh.vertices_begin(); v_iter != v_end; ++v_iter)
		{
			m_mesh.set_point(v_iter, value[j]);
			j++;
		}
	}
}
	...
	if( BLENDING )
	{
		...
		for (int i = 0; i != NR_MIN_PATCH_PER_LEVEL; i++)
		{
			...
			for (int j = 0; j != nr_edges; j++)
			{
				...
				auto p1 = src_mesh.points()[g_from];
				auto p2 = src_mesh.points()[g_to];
				auto q1 = target_mesh.points()[g_from];
				auto q2 = target_mesh.points()[g_to];
				auto l = sqrt((1 - t)*(p1 - p2).norm()*(p1 - p2).norm() + t * (q1 - q2).norm()*(q1 - q2).norm());
				//auto l = (1 - t)*(p1 - p2).norm() + t * (q1 - q2).norm();
				...
				assert(subroot->M.coeffRef(m, from) == 1);
				assert(subroot->M.coeffRef(m, to) == -1);
		...
		MySMatrixXf M1 = subroot->M.block(0, 0, subroot->M.rows(), subroot->M.cols() - 1);
		MySMatrixXf M2 = subroot->M.block(0, subroot->M.cols() - 1, subroot->M.rows(), 1);
		Eigen::SimplicialCholesky<MySMatrixXf > solver;
		solver.compute(M1.transpose()*M1);
		MyMatrixXf result(subroot->pts_index.size(), 3);
		MyMatrixXf c = E - M2 * subroot->pts[subroot->pts_index.size() - 1].transpose();
		MyMatrixXf b = M1.transpose() * (c);
		result.block(0, 0, subroot->pts_index.size() - 1, 3) = solver.solve(b);
		result.block(subroot->pts_index.size() - 1, 0, 1, 3) = subroot->pts[pts_size - 1].transpose();
	/*	std::cout <<"0\t" <<subroot->pts[pts_size - 1].transpose() << std::endl;
		std::cout <<"1\t" <<result(0, 0) <<"\t"<< result(0, 1) << std::endl;*/
...
void MsInterpolation::pre_blending_process( FaceNode * subroot )
{
	...
	subroot->M.resize( nr_cond, nr_vertex);
