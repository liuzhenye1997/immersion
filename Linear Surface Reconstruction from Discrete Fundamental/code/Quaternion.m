    ...
    methods(Static = true)
        ...
        function Q=batch_matrix_to_q(m)
         ...
         function m=q_to_matrix(q)
            ...
            2*x*y+2*w*z 1-2*(x^2+z^2) 2*(y*z-w*x);...
            2*x*z-2*w*y 2*(y*z+w*x) 1-2*(x^2+y^2)];
        end

         function m=batch_q_to_matrix(q)
            x=q(:,2);
            y=q(:,3);
            z=q(:,4);
            w=q(:,1);
            m=zeros(size(q,1)*3,3);
            m(1:3:end,:)=[1-2*(y.^2+z.^2) 2*x.*y-2*w.*z 2*x.*z+2*w.*y];
            m(2:3:end,:)=[2*x.*y+2*w.*z 1-2*(x.^2+z.^2) 2*(y.*z-w.*x)];
            m(3:3:end,:)=[2*x.*z-2*w.*y 2*(y.*z+w.*x) 1-2*(x.^2+y.^2)];
%             m=[1-2*(y^2+z^2) 2*x*y-2*w*z 2*x*z+2*w*y;...
%             2*x*y+2*w*z 1-2*(x^2+z^2) 2*(y*z-w*x);...
%             2*x*z-2*w*y 2*(y*z+w*x) 1-2*(x^2+y^2)];
        end
        ...
        function result=multiplication(p,q)
            result=[p(1)*q(1)-p(2)*q(2)-p(3)*q(3)-p(4)*q(4) p(1)*q(2)+p(2)*q(1)+p(3)*q(4)-p(4)*q(3) p(1)*q(3)-p(2)*q(4)+p(3)*q(1)+p(4)*q(2) p(1)*q(4)+p(2)*q(3)-p(3)*q(2)+p(4)*q(1)];    
        end
