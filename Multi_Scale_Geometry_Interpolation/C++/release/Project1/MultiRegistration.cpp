...
void MultiRegistration::init()
{
	...
	for (int i = 0; i < M; i++)
	{
		R.block(0, 3 * i, 3, 3) = Eigen::MatrixXd::Identity(3, 3);
		//std::cout << R.block(0, 3 * i, 3, 3) << std::endl;
	}
	...
	while( count-- )
	{
		for (int j= 0; j != M; j++)
		{
			...
			{
				R.block<3, 3>(0, j * 3) = MyMatrix3f::Identity();
			}
			else
			{
				R.block<3, 3>(0, j * 3) = V * U.transpose();
			}
		/*	if (R.block<3, 3>(0, j * 3).minCoeff() < -0.5)
			{
				std::cout << svd.singularValues() << std::endl;
				std::cout << Sj << std::endl;
				std::cout << V << std::endl;
				std::cout << U.transpose() << std::endl;
				std::cout << R.block<3, 3>(0, j * 3) << std::endl;
			}*/
	
		}

		MyMatrixXf diff = R - last_R;
		//std::cout << fabs(diff.maxCoeff())<<"\t" << fabs(diff.minCoeff()) << std::endl;
		if( fabs( diff.maxCoeff() ) < 1 * 1e-8 && fabs( diff.minCoeff() ) < 1 * 1e-8 )
		{
			break;
		}
